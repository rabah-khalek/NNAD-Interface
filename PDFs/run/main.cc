...
int main(int argc, char *argv[])
{
  ...
  nnad::FeedForwardNN<double> *nn = new nnad::FeedForwardNN<double>(NNarchitecture, Seed);
...
*/
  ...
  //===print data out
  /*
  ofstream Datatxt("Data.dat", ios::out | ios::app);
  Datatxt << "#x cv sd" << endl;
  for (auto datapoint: Data)
  {
    Datatxt << std::get<0>(datapoint) << " " << std::get<1>(datapoint)<<endl;
  }
  exit(1);
  */
...
*/
  ...
  if (2 * summary.final_cost / totNdata < 1.1)
  {
    ...
    output2 << Seed << "\t" << summary.num_linear_solves << "\t" << 2 * summary.initial_cost / totNdata << "\t" << 2 * summary.final_cost / totNdata << "\t" << t.get() << endl;
  }
  ...
  for (int xi = 0; xi < n; xi++) //[cogged]
    ...
    compute.SetParameters(final_pars);
    std::vector<double> final_predictions = compute.Predictions();

    string theory_OutputFile = ResultsPath + ResultsFolder + "/theory";
    ifstream theory_grids((theory_OutputFile).c_str());
    if (!(theory_grids.good()))
      system(("mkdir " + theory_OutputFile).c_str());
